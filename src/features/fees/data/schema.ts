...
export const createFeeSchema = z
  ...
  .superRefine((data, ctx) => {
    ...
    if (
      ...
      data.min_installments > data.max_installments
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
    ...
    if (data.applies_to === 'USER_SPECIFIC' && !data.user_id) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message:
          'ID do usuário é obrigatório para taxas específicas de usuário',
        path: ['user_id'],
      })
    }
    ...
    if (data.applies_to === 'BRAND_SPECIFIC' && !data.card_brand) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message:
          'Bandeira do cartão é obrigatória para taxas específicas de bandeira',
        path: ['card_brand'],
      })
    }
...
export const calculateFeesSchema = z.object({
  ...
  cardBrand: z
    .enum(['Visa', 'Master', 'Elo', 'Amex', 'Hipercard', 'Diners'])
    .optional(),
  feeTypes: z
    .array(
      z.enum([
